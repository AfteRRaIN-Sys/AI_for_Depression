...
df = pd.read_csv(f'{path}/train_and_validate_all.csv') # Edited by Tom
df.head()
...
#==================================== dataloader ====================================
dbunch = auds.dataloaders(df, bs=16)
dbunch.show_batch(figsize=(10, 5))

#==================================== create learner ====================================
model = EfficientNet.from_pretrained('efficientnet-b0',in_channels=1,num_classes=3)
learn = Learner(dbunch, model,  
            loss_func=CrossEntropyLossFlat(),
            metrics=[accuracy]
            )
#==================================== train model (transfer learning) ====================================
learn.fine_tune( number_of_epochs , cbs=SaveModelCallback() )
...
#==================================== start testing model ====================================
test_csv = f'{master_folder}/all_test.csv' #path to .csv # Edited by Tom
trans = {
  "normal" : 0,
  "sad" : 1,
  "sobbing" : 2,
